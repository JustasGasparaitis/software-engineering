...
countdown num = "Ready! " ++ countdownHelper num "" ++ "Liftoff!"

countdownHelper num result
                | (num == 0) = result
                | otherwise  = (result) ++ (countdownHelper (num - 1) ((show num) ++ "... "))
...
smallestDivisorHelper n x
                      | ((mod n x) == 0) = x
                      | otherwise        = smallestDivisorHelper n (x + 1)
...
biggestPrimeAtMost n
                   | ((isPrime n) == True) = n
                   | otherwise             = biggestPrimeAtMostHelper n (n - 1)

biggestPrimeAtMostHelper n 1 = n
biggestPrimeAtMostHelper n 2 = n
biggestPrimeAtMostHelper n x
                         | ((isPrime x) == True) = x
                         | otherwise             = biggestPrimeAtMostHelper n (x - 1)
