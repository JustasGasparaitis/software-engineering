...
powers :: Int -> Int -> [Int]
powers k max = reverse $ powersList k max 0 []

powersList :: Int -> Int -> Int -> [Int] -> [Int]
powersList k max x list
           | ((k ^ x) > max) = list
           | otherwise       = powersList k max (x + 1) ((k ^ x) : list)
...
joinToLength len strList = [i | i <- (helperList len strList), (length i) == len]
   where helperList len strList = [first ++ last | first <- strList, last <- strList]
...
sumRights strList = sumRightsList (list strList [])

sumRightsList :: [Int] -> Int
sumRightsList [] = 0
sumRightsList (x:xs) = x + sumRightsList xs

list :: [Either a Int] -> [Int] -> [Int]
list [] strList = strList
list (x:xs) strList = case x of
              Left x  -> list (xs) (strList)
              Right x -> list (xs) (strList ++ [x])
...
multiCompose strList val = multiComposeReversed (reverse strList) val

multiComposeReversed :: [(a -> a)] -> a -> a
multiComposeReversed [] val = val
multiComposeReversed (x:xs) val = multiComposeReversed xs (x val)
...
multiApp f gs x = f $ list
   where list = (multiAppList gs x)

multiAppList [] x = []
multiAppList (g:gs) x = [(g x)] ++ (multiAppList gs x)
...
interpreterHelper (c:cs) x y
                  | c == "up"     = (interpreterHelper cs (x) (y + 1))
                  | c == "down"   = (interpreterHelper cs (x) (y - 1))
                  | c == "left"   = (interpreterHelper cs (x - 1) (y))
                  | c == "right"  = (interpreterHelper cs (x + 1) (y))
                  | c == "printX" = [show x] ++ (interpreterHelper cs (x) (y))
                  | c == "printY" = [show y] ++ (interpreterHelper cs (x) (y))
                  | otherwise     = (interpreterHelper cs (x) (y))
