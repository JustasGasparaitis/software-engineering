...
buildList start count end
          | (count == 0) = end : (buildList start count end)
...
sums' :: Int -> Int -> [Int]
sums' 0 j = []
sums' i j = (sums'' j 1) : (sums' (i - 1) (j + 1))

sums'' :: Int -> Int -> Int
sums'' 0 j = 0
sums'' i j = j + (sums'' (i - 1) (j + 1))
...
indexDefault' [] i j def = def
indexDefault' (x:xs) i j def
               | (i == j)                   = x
               | ((isEmpty xs))             = def
               | otherwise                  = indexDefault' xs i (j + 1) def
   where isEmpty :: [a] -> Bool
         isEmpty [] = True
         isEmpty _ = False
...
sumsOf' :: [Int] -> Int -> [Int]
sumsOf' [] y = []
sumsOf' (x:xs) y = (x + y) : sumsOf' xs (x + y)
...
mymaximum' bigger initial [] val = val
mymaximum' bigger initial (x:xs) val
          | (bigger x val) = mymaximum' bigger initial xs x
          | otherwise          = mymaximum' bigger initial xs val
