...
longest xs = (sortBy longest' xs) !! 0
...
winner scores player1 player2
       | (player1Points >= player2Points) = player1
       | otherwise                        = player2
   where player1Points = Map.findWithDefault 0 player1 scores
...
transfer from to amount bank = case ((Map.lookup from bank), (Map.lookup to bank)) of
                               (Nothing, _)                   -> bank
                               (_, Nothing)                   -> bank
                               (Just fromValue, Just toValue)
                                     | (amount > fromValue)
                                        || (amount <= 0)      -> bank
                               (Just fromValue, Just toValue) -> Map.insert from (fromValue - amount) (Map.insert to (toValue + amount) bank)
...
maxIndex' xs = maximumBy (comparing snd) xs
