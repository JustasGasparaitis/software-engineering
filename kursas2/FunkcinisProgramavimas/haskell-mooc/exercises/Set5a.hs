...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix c1 c2) = mixHelper (rgb c1) (rgb c2)
rgb (Invert c) = [ 1 - i | i <- l ]
   where l = rgb c

mixHelper [] [] = []
mixHelper (x:xs) (y:ys) = ((x + y) / 2) : mixHelper xs ys
...
prettyPrint' End = ""
prettyPrint' (O b) = case b of
                    End -> "0"
                    O x -> '0' : prettyPrint' b
                    I x -> '0' : prettyPrint' b
prettyPrint' (I b) = case b of
                    End -> "1"
                    O x -> '1' : prettyPrint' b
                    I x -> '1' : prettyPrint' b

fromBin :: Bin -> Int
fromBin b = fromBin' (reverse (prettyPrint b)) 0

fromBin' [] pow = 0
fromBin' (x:xs) pow
         | x == '1'    = (2 ^ pow) + fromBin' xs (pow + 1)
         | otherwise = fromBin' xs (pow + 1)

toBin :: Int -> Bin
toBin 0 = O End
toBin num = toBin' num 0

toBin' num pow = inc (toBin (num - (2 ^ pow)))