...
import Lib2 (init, render, State, InitData (InitData), gameWidth, gameHeight, update, JsonLike(..))
import Data.Either as E (Either (..))
-- MANDATORY CODE
host :: String
    ...
    ( \sess -> do
        ...
        case toJsonLike initialStr of
          ...
          E.Right j -> do
            ...
            forkIO (doSmth ch (uuid game) sess)
            loop ch (uuid game) sess
            return ()
...
writer :: Control.Concurrent.Chan Lib2.JsonLike -> State -> String -> Sess.Session-> IO()
writer ch state uuid sess = do
  commandsResponse <- readChan ch
  let newState = Lib2.update state commandsResponse
  draw newState
  writer ch newState uuid sess

doSmth :: Control.Concurrent.Chan Lib2.JsonLike -> String -> Sess.Session -> IO()
doSmth ch uuid sess = do
...
loop :: Control.Concurrent.Chan Lib2.JsonLike -> String -> Sess.Session -> IO()
loop ch uuid sess = do
  c <- getChar
...
getAllInfo = Just (Commands FetchBombSurrounding(Just (Commands FetchBombStatus (Just (Commands FetchSurrounding Nothing )))))